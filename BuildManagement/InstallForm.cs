...
namespace BuildManagement
{
    public partial class InstallForm : Form
    {
            ...
            switch (settingsModel.Other.Mode)
            {
                ...
                case "Kyle":
                    ...
                    if (pathSplit.Count() > 2)
                        newPath = String.Format(@"{0} {1}", defaultPath, pathFromInstaller.Substring(Utils.GetNthIndex(pathFromInstaller, '\\', pathSplit.Count() - 2)));
                    else
                        newPath = String.Format(@"{0} {1}", defaultPath, pathFromInstaller);
                    break;
                case "SmartBear":
        ...
        public void InstallGPWebBuild()
        {
            ...
            string tempInstaller = String.Format(@"{0}\{1}", Utils.GetFolder("Installers"), installerFileName);
        ...
        public void InstallBuild()
        {
            ...
            catch (Exception e)
            {
                ...
                ErrorHandle.DisplayExceptionMessage(e, false, null, action, variables);
            }
            ...
            {
                ErrorHandle.LogException(deleteError);
                ErrorHandle.DisplayExceptionMessage(deleteError);
            }
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            DirectoryInfo di = new DirectoryInfo(Utils.GetFolder("Installers"));
            ...
            if (Directory.Exists(installPath))
            {
                if (this.Product != Products.GPWeb || this.Product != Products.WebAPI)
                {
                    ...
                    if (existsResult == DialogResult.Yes)
                    {
                        ...
                        {
                            ErrorHandle.LogException(ex);
                            ErrorHandle.DisplayExceptionMessage(ex);
                            return;
                        }
                    }
                    else
