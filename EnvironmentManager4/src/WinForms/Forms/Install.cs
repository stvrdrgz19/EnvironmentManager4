    ...
    public partial class Install : Form
    {
        ...
        public static string GetInstallerPath(string path, string product, string version)
        {
            ...
            using (OpenFileDialog openFileDialog = new OpenFileDialog())
            {
                ...
                else if (product == ProductsOld.ShipCenter)
                {
                    switch (version)
                    {
                        case "x64":
                ...
                else if (product == ProductsOld.GPWeb)
                {
                    openFileDialog.Filter = "ZIP Folder (.zip)|*.zip";
                }
                else if (product == ProductsOld.WebAPI)
                {
            ...
            else
            {
                switch (this.Product)
                {
                    case ProductsOld.SalesPad:
                        ...
                        {
                            case "x64":
                                path = settings.BMSalesPadx64;
                                break;
                            case "x86":
                            case "Pre":
                                path = settings.BMSalesPadx86;
                                break;
                        }
                        break;
                    case ProductsOld.DataCollection:
                        charCount = 51;
                        path = settings.BMDataCollection;
                        break;
                    case ProductsOld.SalesPadMobile:
                        charCount = 50;
                        path = settings.BMSalesPadMobile;
                        break;
                    case ProductsOld.ShipCenter:
                        ...
                        {
                            case "x64":
                                path = settings.BMShipCenterx64;
                                break;
                            case "x86":
                            case "Pre":
                                path = settings.BMShipCenterx86;
                                break;
                        }
                        break;
                }
        ...
        public static string GetInstallPath(string defaultPath, string installerPath, int charCount)
        {
            ...
            // TODO: Evaluate if the Mode and Default versions settings can be removed
        ...
        public void InstallGPWebBuild()
        {
            ...
            string tempInstaller = String.Format(@"{0}\{1}", DirectoryUtilities.GetFolder("Installers"), installerFileName);
        ...
        {
            // establish if toast notifications are enabled in settings
            AppSettings settings = new AppSettings();
            bool pushToast = settings.OEnableInstallToasts;
            ...
            if (custDllToAdd.Count > 0 || extDllToAdd.Count > 0)
            {
                if (this.Product == ProductsOld.DataCollection)
                {
        ...
        private void Install_Load(object sender, EventArgs e)
        {
            ...
            if (this.Product != ProductsOld.SalesPad)
            {
                checkRunDatabaseUpdate.Enabled = false;
            }
            if (this.Product == ProductsOld.SalesPadMobile)
            {
                cbConfigurationList.Enabled = false;
                btnAddConfiguration.Enabled = false;
                btnRemoveConfiguration.Enabled = false;
            }
            if (this.Product == ProductsOld.WebAPI || this.Product == ProductsOld.GPWeb)
            {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            DirectoryInfo di = new DirectoryInfo(DirectoryUtilities.GetFolder("Installers"));
            ...
            {
                if (this.Product != ProductsOld.GPWeb || this.Product != ProductsOld.WebAPI)
                {
